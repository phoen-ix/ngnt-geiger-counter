...
Last updated: 2026-02-24 (container versions bumped; unused PHP extensions removed; async subscriber with batching + pool; DB partitioning)
...
- Credentials and server address are **hardcoded constants** near the top (Arduino has no env system). They must be kept in sync with `.env` manually.
...
| MQTT server address + credentials | `geiger_counter_v2.0.ino` lines 55–58 | Must match `.env` manually |
| WiFi AP password | `geiger_counter_v2.0.ino` line 50 | Only used during initial WiFi setup |
